...
function [s,t] = filter_sync(s,t,varargin)
...
ind_end = length(s(:,1,:));
[~,ind_max] = max(s); % Find index of max in time
ind_sync = ind_max;
...
temp = zeros(ind_end-ind_absmax,length(s(1,:,1)),...
    length(s(1,1,:)));
for jj=1:length(temp(1,:,1)) % shot loop
    for ii=1:length(temp(1,1,:)) % wavelength loop
        temp(:,jj,ii) = s(ind_sync(1,jj,ii):(ind_end-ind_absmax+ind_sync(1,jj,ii)-1),jj,ii);
    end
end
s = temp;

t = t(1:(ind_end-ind_absmax));
